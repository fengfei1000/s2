...
import fengfei.controllers.Admin;
    ...
    public boolean preHandle(
        ...
        if (session.getAttribute(Admin.SESSION_LOGIN_KEY) == null) {
            ...
            if (responseType == ResponseType.HtmlPage) {
                ...
                Done done = new Done("Please login!", Status.Fail);
        ...
        boolean aflag = false;
        User user = (User) session.getAttribute(Admin.SESSION_USER_KEY);
        String rights = (String) session.getAttribute(Admin.SESSION_USER_AUTH_KEY);
        ...
        if (false == aflag) {
            ...
            if (responseType == ResponseType.HtmlPage) {
                ...
                OutputStream out = response.getOutputStream();
                PrintWriter pw = new PrintWriter(new OutputStreamWriter(out, "utf-8"));
                Done done = new Done("Can't be operated. No Authority!", Status.Fail);
                String json = mapper.writeValueAsString(done);
                pw.println(json);
                pw.flush();
                pw.close();
