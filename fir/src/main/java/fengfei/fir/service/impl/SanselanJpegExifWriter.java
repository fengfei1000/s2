...
public class SanselanJpegExifWriter implements JpegExifWriter,
		SanselanExifTagConstants {
	...
	private void writeExif(String imageFile, boolean isReadOriginTags,
		...
		for (Entry<String, String> entry : values.entrySet()) {
			...
			if (tagInfo != null) {
				...
				TiffOutputDirectory directory = getOrCreateDirectory(tagInfo,
						outputSet);
				System.out.printf("%s %s %s \n", directory, tagInfo, outputSet);
				if (directory != null) {
					byte[] bs = convertValue(tagInfo, outputSet, value);
					if (bs != null) {
						TiffOutputField field = new TiffOutputField(tagInfo,
								tagInfo.dataTypes[0], bs.length, bs);
	...
	private TiffOutputDirectory getOrCreateDirectory(TagInfo tagInfo,
	...
	private byte[] convertValue(TagInfo tagInfo, TiffOutputSet outputSet,
